...
CREATE FUNCTION `fn_create_random`() RETURNS VARCHAR(128) CHARSET utf8 DETERMINISTIC
  BEGIN
    SET @random = CONCAT(
...
CREATE FUNCTION `fn_create_session`() RETURNS VARCHAR(128) CHARSET utf8 DETERMINISTIC
  BEGIN
...
CREATE FUNCTION `fn_create_session_length`(vLENGTH INT) RETURNS VARCHAR(128) CHARSET utf8 DETERMINISTIC
  BEGIN
    SET @chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890';
    SET @charLen = LENGTH(@chars);
    SET @random = '';
...
CREATE FUNCTION `fn_create_url`() RETURNS VARCHAR(128) CHARSET utf8 DETERMINISTIC
	BEGIN
